...
═══════════════════════════════════════════════════════════════════════════════
    ...
    1. Optimisation des paramètres SMA (Simple Moving Averages)
    2. Stratégie SMA optimisée avec backtesting vectorisé
    3. Stratégie basée sur la régression (OLS)
    4. Stratégie de Machine Learning (K-Means Clustering amélioré)

    Approche personnalisée selon le profil d'investisseur
═══════════════════════════════════════════════════════════════════════════════
"""
...
# Bibliothèques pour graphiques interactifs
...
# ═══════════════════════════════════════════════════════════════════════════
# SECTION 2: RÉCUPÉRATION DES DONNÉES NINTENDO
# ═══════════════════════════════════════════════════════════════════════════
...
print(f"✅ Données téléchargées: {len(data_original)} jours de trading")
print(f"   Période: {data_original.index[0].strftime('%Y-%m-%d')} à {data_original.index[-1].strftime('%Y-%m-%d')}")
...
print("=" * 80)
print()
print("📋 Choisissez votre profil d'investisseur:")
print("   1. CONSERVATEUR : Minimisation du risque, fréquence de trading faible")
print("   2. MODÉRÉ : Équilibre risque/rendement, fréquence de trading moyenne")
print("   3. DYNAMIQUE : Maximisation du rendement, fréquence de trading élevée")
print("   4. PERSONNALISÉ : Définir manuellement les paramètres")
print()

# Configuration par défaut (modifiable)
PROFIL_CHOISI = 2  # Par défaut ("2" = Modéré)
...
profils = {
    1: {
        ...
        'n_clusters_kmeans': 2,
        'description': 'Stratégies à faible volatilité, réaction lente aux changements de marché'
    },
    2: {
        'nom': 'MODÉRÉ',
        ...
        'n_lags_regression': 7,
        'n_clusters_kmeans': 3,
        'description': 'Équilibre entre réactivité et stabilité'
    },
    3: {
        'nom': 'DYNAMIQUE',
        ...
        'n_lags_regression': 5,
        'n_clusters_kmeans': 4,
        'description': 'Stratégies réactives, plus de signaux de trading'
    }
}

# --- Cas du profil 4 : entrée manuelle ---
if PROFIL_CHOISI == 4:
    print("Mode personnalisé activé :")
    # Saisie paramètres personnalisés par l'utilisateur

    try:
        sma_short_deb = int(input("Début de la plage du SMA court (ex: 10) : "))
        sma_short_fin = int(input("Fin de la plage du SMA court (ex: 50) : "))
        sma_short_pas = int(input("Pas du SMA court (ex: 5) : "))

        sma_long_deb = int(input("Début de la plage du SMA long (ex: 100) : "))
        sma_long_fin = int(input("Fin de la plage du SMA long (ex: 200) : "))
        sma_long_pas = int(input("Pas du SMA long (ex: 10) : "))

        n_lags_regression = int(input("Nombre de lags régression OLS (ex: 5) : "))
        n_clusters_kmeans = int(input("Nombre de clusters K-means (ex: 4) : "))
    except Exception as e:
        print("\n⚠️ Problème de saisie. Valeurs par défaut utilisées !\n", e)
    ...
    profil = {
        ...
        'description': 'Paramètres définis manuellement'
    }
...
print(f"✅ Profil sélectionné: {profil['nom']}")
print(f"   Description: {profil['description']}")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 4: OPTIMISATION DES PARAMÈTRES SMA
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  OPTIMISATION DES PARAMÈTRES SMA")
print("=" * 80)
print()

print(f"🔍 Recherche des meilleurs paramètres SMA pour profil {profil['nom']}...")
...
print(f"✅ {len(optimization_results)} combinaisons testées")
print()
print("🏆 TOP 10 DES MEILLEURES COMBINAISONS (par Sharpe Ratio):")
print("-" * 80)
...
print(f"⭐ PARAMÈTRES OPTIMAUX SÉLECTIONNÉS:")
print(f"   - SMA Court: {SMA_SHORT_OPT} jours")
print(f"   - SMA Long: {SMA_LONG_OPT} jours")
print(f"   - Performance Stratégie: {best_params['Strategy_Return']:.4f}x ({(best_params['Strategy_Return']-1)*100:.2f}%)")
...
fig_opt.update_layout(
    title=f'Optimisation SMA - Profil {profil["nom"]}: Sharpe Ratio selon les paramètres',
    scene=dict(
...
fig_opt.show()
print("📊 Graphique interactif: Surface d'optimisation 3D générée")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 5: STRATÉGIE SMA OPTIMISÉE
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print(f"  STRATÉGIE SMA OPTIMISÉE (SMA {SMA_SHORT_OPT}/{SMA_LONG_OPT})")
print("=" * 80)
print()
...
print(f"📊 Paramètres SMA optimaux appliqués:")
...
fig_sma = make_subplots(
    ...
    subplot_titles=(f'Prix Nintendo et SMAs Optimisées ({SMA_SHORT_OPT}/{SMA_LONG_OPT})',
                    'Signaux de Trading'),
    row_heights=[0.7, 0.3]
)
...
fig_sma.update_layout(
    height=800,
    title_text=f"Stratégie SMA Optimisée - Profil {profil['nom']}",
    hovermode='x unified'
)

fig_sma.show()
print("📊 Graphique interactif: SMAs et signaux de trading générés")
print()
...
print("=" * 80)
print("  BACKTESTING VECTORISÉ - COMPARAISON BUY & HOLD VS SMA")
print("=" * 80)
print()
...
print("-" * 80)
print(f"{'Métrique':<40} {'Buy & Hold':<20} {'SMA Strategy':<20}")
print("-" * 80)
print(f"{'Performance cumulée':<40} {cumulative_returns['Returns']:<20.4f} {cumulative_returns['Strategy']:<20.4f}")
...
print(f"{'Volatilité annualisée':<40} {annualized_volatility['Returns']:<20.4f} {annualized_volatility['Strategy']:<20.4f}")
...
print(f"{'Durée (années)':<40} {years:<20.2f}")
print("-" * 80)
print()
...
fig_backtest.update_layout(
    title=f'Performance Cumulative: Buy & Hold vs Stratégie SMA Optimisée<br>Profil {profil["nom"]} - SMA {SMA_SHORT_OPT}/{SMA_LONG_OPT}',
    xaxis_title='Date',
...
fig_backtest.show()
print("📊 Graphique interactif: Comparaison des performances générée")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 7: STRATÉGIE BASÉE SUR LA RÉGRESSION (OLS)
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  STRATÉGIE BASÉE SUR LA RÉGRESSION (OLS)")
print("=" * 80)
print()
...
# Création des lags selon le profil
...
print(f"📊 Régression OLS avec {lags} lags (profil {profil['nom']})")
print(f"   Nombre d'observations: {len(data_regression)}")
print()

# Modèle 1: Régression sur les rendements
...
# Modèle 2: Régression sur la direction
...
print("📊 RÉSULTATS DES STRATÉGIES DE RÉGRESSION:")
print("-" * 80)
print(f"{'Stratégie':<40} {'Performance':<20} {'% Gain':<20}")
print("-" * 80)
print(f"{'Buy & Hold':<40} {perf_regression['returns']:<20.4f} {(perf_regression['returns']-1)*100:<20.2f}%")
print(f"{'Régression sur Rendements':<40} {perf_regression['strat_reg_returns']:<20.4f} {(perf_regression['strat_reg_returns']-1)*100:<20.2f}%")
print(f"{'Régression sur Direction':<40} {perf_regression['strat_reg_direction']:<20.4f} {(perf_regression['strat_reg_direction']-1)*100:<20.2f}%")
print("-" * 80)
print()
print(f"{'Précision des Prédictions:':<40}")
print(f"{'  Modèle sur Rendements':<40} {accuracy_returns*100:<20.2f}%")
print(f"{'  Modèle sur Direction':<40} {accuracy_direction*100:<20.2f}%")
print()
...
fig_reg.add_trace(go.Scatter(x=cumulative_reg.index, y=cumulative_reg['strat_reg_returns'],
                             name='Régression Rendements', line=dict(color='orange', width=2)))
fig_reg.add_trace(go.Scatter(x=cumulative_reg.index, y=cumulative_reg['strat_reg_direction'],
                             name='Régression Direction', line=dict(color='green', width=2)))

fig_reg.update_layout(
    title=f'Performances des Stratégies de Régression OLS<br>Profil {profil["nom"]} - {lags} lags',
    xaxis_title='Date',
    yaxis_title='Performance Cumulative',
    hovermode='x unified',
    height=600
...
fig_reg.show()
print("📊 Graphique interactif: Performances régression générées")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 8: K-MEANS CLUSTERING AMÉLIORÉ (MACHINE LEARNING)
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  K-MEANS CLUSTERING AMÉLIORÉ (MACHINE LEARNING)")
print("=" * 80)
print()
...
# Ajout de features techniques supplémentaires
...
print(f"📊 K-Means Clustering avec {n_clusters} clusters (profil {profil['nom']})")
print(f"   Features utilisées: {len(features_for_clustering)} (lags + volatilité + momentum)")
print()

# Analyse des clusters pour déterminer les positions
cluster_returns = data_ml.groupby('cluster')['returns'].mean()
print("📊 Rendements moyens par cluster:")
for cluster, ret in cluster_returns.items():
    print(f"   Cluster {cluster}: {ret*100:.4f}% {'(Favorable)' if ret > 0 else '(Défavorable)'}")
print()

# Attribution des positions selon la performance des clusters
# Les clusters avec rendements positifs → Long (1)
# Les clusters avec rendements négatifs → Short (-1)
...
print("📊 RÉSULTATS DU K-MEANS CLUSTERING:")
print("-" * 80)
print(f"{'Stratégie':<40} {'Performance':<20} {'% Gain':<20}")
print("-" * 80)
...
print("-" * 80)
print()
print(f"{'Précision des Prédictions:':<40} {accuracy_cluster*100:<20.2f}%")
...
fig_clusters.show()
print("📊 Graphique interactif: Clusters et performance générés")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 9: TABLEAU RÉCAPITULATIF ET RECOMMANDATIONS
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  TABLEAU RÉCAPITULATIF - TOUTES LES STRATÉGIES")
print("=" * 80)
print()
...
summary = pd.DataFrame({
    'Stratégie': [
        'Buy & Hold',
        f'SMA Optimisée ({SMA_SHORT_OPT}/{SMA_LONG_OPT})',
        f'Régression Rendements ({lags} lags)',
        f'Régression Direction ({lags} lags)',
...
strategies = summary['Stratégie'].tolist()
...
fig_final.show()
print("📊 Graphique interactif: Comparaison finale générée")
print()
...
print(f"🎯 MEILLEURE STRATÉGIE pour profil {profil['nom']}: {best_strategy['Stratégie']}")
print(f"   ├─ Performance: {best_strategy['Performance Cumulative']:.4f}x ({best_strategy['Performance %']:.2f}%)")
print(f"   ├─ Surperformance vs Buy&Hold: {best_strategy['Gain vs Buy&Hold %']:.2f}%")
print(f"   ├─ Sharpe Ratio: {best_strategy['Sharpe Ratio']:.4f}")
print(f"   ├─ Volatilité annualisée: {best_strategy['Volatilité']:.4f}")
print(f"   └─ Nombre de trades: {int(best_strategy['Nombre de Trades'])}")
print()

print("💡 CONSEILS D'INVESTISSEMENT:")
print(f"   1. Pour un profil {profil['nom']}, la stratégie recommandée offre un bon équilibre risque/rendement")
print(f"   2. La stratégie SMA optimisée ({SMA_SHORT_OPT}/{SMA_LONG_OPT}) est robuste et facile à implémenter")
print(f"   3. Les stratégies de Machine Learning offrent de la diversification")
print(f"   4. Toujours considérer les coûts de transaction dans l'implémentation réelle")
print(f"   5. Ces résultats sont basés sur des données historiques - les performances passées ne garantissent pas les performances futures")
print()
