# -*- coding: utf-8 -*-
"""Nintendo - Option pricing.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1EtGxPnckwLWK-mOxy91Cr0iEpA-WaIph

═══════════════════════════════════════════════════════════════════════════════
    OPTION PRICING STRATEGIES FOR NINTENDO (NTDOY)
    Période: Septembre 2015 - Septembre 2025

    Options évaluées:
    1. Options Européennes (Black-Scholes-Merton)
    2. Options Américaines (Binomial Tree)
    3. Options Bermudéennes (Binomial Tree modifié)
    4. Options Exotiques - Asiatiques (Monte Carlo)
═══════════════════════════════════════════════════════════════════════════════
""
"""

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 1: IMPORTS ET CONFIGURATION
# ═══════════════════════════════════════════════════════════════════════════

import numpy as np
import pandas as pd
import yfinance as yf
import matplotlib.pyplot as plt
import seaborn as sns
from scipy.stats import norm
from datetime import datetime
import warnings
warnings.filterwarnings('ignore')

# Configuration des graphiques
plt.style.use('seaborn-v0_8-whitegrid')
sns.set_palette("Set2")
plt.rcParams['figure.figsize'] = (14, 8)
plt.rcParams['font.size'] = 11

print("=" * 80)
print("  OPTION PRICING STRATEGIES - NINTENDO (NTDOY)")
print("=" * 80)
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 2: RÉCUPÉRATION ET ANALYSE DES DONNÉES
# ═══════════════════════════════════════════════════════════════════════════

print("📥 Téléchargement des données Nintendo...")

ticker = "NTDOY"
start_date = "2015-09-01"
end_date = "2025-09-30"

# Téléchargement des données
nintendo = yf.download(ticker, start=start_date, end=end_date)
prices = nintendo['Close'].dropna()

# Calcul des paramètres du marché
log_returns = np.log(prices / prices.shift(1)).dropna()
current_price = prices.iloc[-1].item() # Extract scalar value
volatility = log_returns.std().item() * np.sqrt(252)  # Volatilité annualisée
mean_return = log_returns.mean().item() * 252  # Rendement moyen annualisé

print(f"✅ Analyse des données Nintendo:")
print(f"   - Prix actuel (S₀): ${current_price:.2f}")
print(f"   - Volatilité annualisée (σ): {volatility:.4f} ({volatility*100:.2f}%)")
print(f"   - Rendement moyen annualisé (μ): {mean_return:.4f} ({mean_return*100:.2f}%)")
print(f"   - Période d'analyse: {len(prices)} jours de trading")
print()

# Paramètres des options
strike_price = current_price  # At-the-money (ATM)
time_to_maturity = 1.0  # 1 an
risk_free_rate = 0.045  # Taux sans risque ~4.5% (à ajuster selon le marché actuel)

print(f"📋 PARAMÈTRES DES OPTIONS:")
print(f"   - Prix d'exercice (K): ${strike_price:.2f} (At-The-Money)")
print(f"   - Maturité (T): {time_to_maturity} an(s)")
print(f"   - Taux sans risque (r): {risk_free_rate:.3f} ({risk_free_rate*100:.1f}%)")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 3: OPTIONS EUROPÉENNES - BLACK-SCHOLES-MERTON
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  OPTION 1: OPTIONS EUROPÉENNES (Black-Scholes-Merton)")
print("=" * 80)
print()

def black_scholes_european(S, K, T, r, sigma, option_type='call'):
    """
    Calcul du prix d'une option européenne par Black-Scholes-Merton

    Paramètres:
        S: Prix actuel de l'actif
        K: Prix d'exercice
        T: Temps jusqu'à maturité (années)
        r: Taux sans risque
        sigma: Volatilité
        option_type: 'call' ou 'put'

    Retourne:
        Prix de l'option
    """
    d1 = (np.log(S / K) + (r + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)

    if option_type == 'call':
        price = S * norm.cdf(d1) - K * np.exp(-r * T) * norm.cdf(d2)
    else:  # put
        price = K * np.exp(-r * T) * norm.cdf(-d2) - S * norm.cdf(-d1)

    return price

# Calcul des prix des options européennes
european_call = black_scholes_european(
    current_price, strike_price, time_to_maturity,
    risk_free_rate, volatility, 'call'
)

european_put = black_scholes_european(
    current_price, strike_price, time_to_maturity,
    risk_free_rate, volatility, 'put'
)

print("📊 PRIX DES OPTIONS EUROPÉENNES:")
print("-" * 80)
print(f"{'Type Option':<30} {'Prix':<20} {'% du Spot':<20}")
print("-" * 80)
print(f"{'Call Européen':<30} ${european_call:<19.4f} {european_call/current_price*100:<19.2f}%")
print(f"{'Put Européen':<30} ${european_put:<19.4f} {european_put/current_price*100:<19.2f}%")
print("-" * 80)
print()

# Vérification de la parité Put-Call
put_call_parity_check = european_call - european_put - (current_price - strike_price * np.exp(-risk_free_rate * time_to_maturity))
print(f"✅ Vérification Parité Put-Call: {abs(put_call_parity_check):.10f} (doit être ≈ 0)")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 4: OPTIONS AMÉRICAINES - BINOMIAL TREE
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  OPTION 2: OPTIONS AMÉRICAINES (Binomial Tree)")
print("=" * 80)
print()

def binomial_tree_american(S, K, T, r, sigma, N, option_type='call'):
    """
    Calcul du prix d'une option américaine par arbre binomial

    Paramètres:
        S: Prix actuel de l'actif
        K: Prix d'exercice
        T: Temps jusqu'à maturité (années)
        r: Taux sans risque
        sigma: Volatilité
        N: Nombre de pas dans l'arbre
        option_type: 'call' ou 'put'

    Retourne:
        Prix de l'option
    """
    dt = T / N
    u = np.exp(sigma * np.sqrt(dt))  # Facteur de hausse
    d = 1 / u  # Facteur de baisse
    p = (np.exp(r * dt) - d) / (u - d)  # Probabilité risque-neutre

    # Initialisation de l'arbre des prix
    price_tree = np.zeros((N + 1, N + 1))

    # Prix à maturité
    for i in range(N + 1):
        price_tree[i, N] = S * (u ** (N - i)) * (d ** i)

    # Valeurs de l'option à maturité
    option_tree = np.zeros((N + 1, N + 1))

    for i in range(N + 1):
        if option_type == 'call':
            option_tree[i, N] = max(0, price_tree[i, N] - K)
        else:  # put
            option_tree[i, N] = max(0, K - price_tree[i, N])

    # Backward induction
    for j in range(N - 1, -1, -1):
        for i in range(j + 1):
            price_tree[i, j] = S * (u ** (j - i)) * (d ** i)

            # Valeur de continuation
            continuation_value = np.exp(-r * dt) * (
                p * option_tree[i, j + 1] + (1 - p) * option_tree[i + 1, j + 1]
            )

            # Valeur d'exercice immédiat
            if option_type == 'call':
                exercise_value = max(0, price_tree[i, j] - K)
            else:  # put
                exercise_value = max(0, K - price_tree[i, j])

            # Pour les options américaines: max(continuation, exercice)
            option_tree[i, j] = max(continuation_value, exercise_value)

    return option_tree[0, 0]

# Nombre de pas dans l'arbre binomial
N_steps = 100

print(f"🌳 Arbre binomial avec {N_steps} pas")
print()

# Calcul des prix des options américaines
american_call = binomial_tree_american(
    current_price, strike_price, time_to_maturity,
    risk_free_rate, volatility, N_steps, 'call'
)

american_put = binomial_tree_american(
    current_price, strike_price, time_to_maturity,
    risk_free_rate, volatility, N_steps, 'put'
)

print("📊 PRIX DES OPTIONS AMÉRICAINES:")
print("-" * 80)
print(f"{'Type Option':<30} {'Prix':<20} {'% du Spot':<20}")
print("-" * 80)
print(f"{'Call Américain':<30} ${american_call:<19.4f} {american_call/current_price*100:<19.2f}%")
print(f"{'Put Américain':<30} ${american_put:<19.4f} {american_put/current_price*100:<19.2f}%")
print("-" * 80)
print()

# Prime d'exercice anticipé
early_exercise_premium_call = american_call - european_call
early_exercise_premium_put = american_put - european_put

print("💰 PRIME D'EXERCICE ANTICIPÉ:")
print(f"   - Call: ${early_exercise_premium_call:.4f} ({early_exercise_premium_call/european_call*100:.2f}%)")
print(f"   - Put: ${early_exercise_premium_put:.4f} ({early_exercise_premium_put/european_put*100:.2f}%)")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 5: OPTIONS BERMUDÉENNES
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  OPTION 3: OPTIONS BERMUDÉENNES (Binomial Tree modifié)")
print("=" * 80)
print()

def binomial_tree_bermudan(S, K, T, r, sigma, N, exercise_dates, option_type='call'):
    """
    Calcul du prix d'une option bermudéenne par arbre binomial

    Les options bermudéennes peuvent être exercées uniquement à des dates spécifiques

    Paramètres:
        S: Prix actuel de l'actif
        K: Prix d'exercice
        T: Temps jusqu'à maturité (années)
        r: Taux sans risque
        sigma: Volatilité
        N: Nombre de pas dans l'arbre
        exercise_dates: Liste des dates d'exercice possibles (en fraction de T)
        option_type: 'call' ou 'put'

    Retourne:
        Prix de l'option
    """
    dt = T / N
    u = np.exp(sigma * np.sqrt(dt))
    d = 1 / u
    p = (np.exp(r * dt) - d) / (u - d)

    # Conversion des dates d'exercice en indices
    exercise_indices = [int(date * N) for date in exercise_dates]
    exercise_indices.append(N)  # On peut toujours exercer à maturité

    # Initialisation
    price_tree = np.zeros((N + 1, N + 1))
    option_tree = np.zeros((N + 1, N + 1))

    # Prix à maturité
    for i in range(N + 1):
        price_tree[i, N] = S * (u ** (N - i)) * (d ** i)
        if option_type == 'call':
            option_tree[i, N] = max(0, price_tree[i, N] - K)
        else:
            option_tree[i, N] = max(0, K - price_tree[i, N])

    # Backward induction
    for j in range(N - 1, -1, -1):
        for i in range(j + 1):
            price_tree[i, j] = S * (u ** (j - i)) * (d ** i)

            # Valeur de continuation
            continuation_value = np.exp(-r * dt) * (
                p * option_tree[i, j + 1] + (1 - p) * option_tree[i + 1, j + 1]
            )

            # Valeur d'exercice
            if option_type == 'call':
                exercise_value = max(0, price_tree[i, j] - K)
            else:
                exercise_value = max(0, K - price_tree[i, j])

            # Si c'est une date d'exercice possible, on prend le max
            if j in exercise_indices:
                option_tree[i, j] = max(continuation_value, exercise_value)
            else:
                option_tree[i, j] = continuation_value

    return option_tree[0, 0]

# Dates d'exercice possibles (trimestriellement: 0.25, 0.5, 0.75, 1.0)
exercise_dates = [0.25, 0.5, 0.75]

print(f"📅 Dates d'exercice possibles: trimestres {[f'{d*12:.0f} mois' for d in exercise_dates]} + maturité")
print()

# Calcul des prix des options bermudéennes
bermudan_call = binomial_tree_bermudan(
    current_price, strike_price, time_to_maturity,
    risk_free_rate, volatility, N_steps, exercise_dates, 'call'
)

bermudan_put = binomial_tree_bermudan(
    current_price, strike_price, time_to_maturity,
    risk_free_rate, volatility, N_steps, exercise_dates, 'put'
)

print("📊 PRIX DES OPTIONS BERMUDÉENNES:")
print("-" * 80)
print(f"{'Type Option':<30} {'Prix':<20} {'% du Spot':<20}")
print("-" * 80)
print(f"{'Call Bermudéen':<30} ${bermudan_call:<19.4f} {bermudan_call/current_price*100:<19.2f}%")
print(f"{'Put Bermudéen':<30} ${bermudan_put:<19.4f} {bermudan_put/current_price*100:<19.2f}%")
print("-" * 80)
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 6: OPTIONS ASIATIQUES - MONTE CARLO
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  OPTION 4: OPTIONS EXOTIQUES - ASIATIQUES (Monte Carlo)")
print("=" * 80)
print()

def asian_option_monte_carlo(S, K, T, r, sigma, N_steps, N_sims, option_type='call'):
    """
    Calcul du prix d'une option asiatique par Monte Carlo

    L'option asiatique dépend du prix moyen de l'actif pendant la durée de vie de l'option

    Paramètres:
        S: Prix actuel de l'actif
        K: Prix d'exercice
        T: Temps jusqu'à maturité (années)
        r: Taux sans risque
        sigma: Volatilité
        N_steps: Nombre de pas de temps dans la simulation
        N_sims: Nombre de simulations Monte Carlo
        option_type: 'call' ou 'put'

    Retourne:
        Prix de l'option
    """
    dt = T / N_steps
    discount_factor = np.exp(-r * T)

    # Simulations des trajectoires de prix
    payoffs = np.zeros(N_sims)

    for sim in range(N_sims):
        # Génération d'une trajectoire
        prices = np.zeros(N_steps + 1)
        prices[0] = S

        for t in range(1, N_steps + 1):
            z = np.random.standard_normal()
            prices[t] = prices[t-1] * np.exp(
                (r - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * z
            )

        # Prix moyen arithmétique
        average_price = np.mean(prices)

        # Payoff
        if option_type == 'call':
            payoffs[sim] = max(0, average_price - K)
        else:  # put
            payoffs[sim] = max(0, K - average_price)

    # Prix de l'option
    option_price = discount_factor * np.mean(payoffs)
    std_error = discount_factor * np.std(payoffs) / np.sqrt(N_sims)

    return option_price, std_error

# Paramètres Monte Carlo
N_mc_steps = 252  # Une observation par jour de trading
N_simulations = 10000

print(f"🎲 Simulation Monte Carlo:")
print(f"   - Nombre de simulations: {N_simulations:,}")
print(f"   - Pas de temps: {N_mc_steps}")
print()

# Seed pour reproductibilité
np.random.seed(42)

# Calcul des prix des options asiatiques
asian_call, asian_call_se = asian_option_monte_carlo(
    current_price, strike_price, time_to_maturity,
    risk_free_rate, volatility, N_mc_steps, N_simulations, 'call'
)

asian_put, asian_put_se = asian_option_monte_carlo(
    current_price, strike_price, time_to_maturity,
    risk_free_rate, volatility, N_mc_steps, N_simulations, 'put'
)

print("📊 PRIX DES OPTIONS ASIATIQUES:")
print("-" * 80)
print(f"{'Type Option':<30} {'Prix':<20} {'Écart-type':<20}")
print("-" * 80)
print(f"{'Call Asiatique':<30} ${asian_call:<19.4f} ±${asian_call_se:<19.6f}")
print(f"{'Put Asiatique':<30} ${asian_put:<19.4f} ±${asian_put_se:<19.6f}")
print("-" * 80)
print()

# Intervalle de confiance 95%
print("📈 INTERVALLES DE CONFIANCE (95%):")
print(f"   - Call: ${asian_call - 1.96*asian_call_se:.4f} - ${asian_call + 1.96*asian_call_se:.4f}")
print(f"   - Put: ${asian_put - 1.96*asian_put_se:.4f} - ${asian_put + 1.96*asian_put_se:.4f}")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 7: COMPARAISON GLOBALE DES PRIX
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  COMPARAISON GLOBALE DES PRIX D'OPTIONS")
print("=" * 80)
print()

# Création du DataFrame de comparaison
comparison = pd.DataFrame({
    'Type d\'Option': [
        'Call Européen',
        'Call Américain',
        'Call Bermudéen',
        'Call Asiatique',
        'Put Européen',
        'Put Américain',
        'Put Bermudéen',
        'Put Asiatique'
    ],
    'Prix ($)': [
        european_call,
        american_call,
        bermudan_call,
        asian_call,
        european_put,
        american_put,
        bermudan_put,
        asian_put
    ],
    '% du Prix Spot': [
        european_call/current_price*100,
        american_call/current_price*100,
        bermudan_call/current_price*100,
        asian_call/current_price*100,
        european_put/current_price*100,
        american_put/current_price*100,
        bermudan_put/current_price*100,
        asian_put/current_price*100
    ]
})

print(comparison.to_string(index=False))
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 8: VISUALISATIONS
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  GÉNÉRATION DES VISUALISATIONS")
print("=" * 80)
print()

# Graphique 1: Comparaison des prix par type
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

# Calls
call_data = comparison[comparison['Type d\'Option'].str.contains('Call')]
ax1.bar(range(len(call_data)), call_data['Prix ($)'], color=['#3498db', '#e74c3c', '#f39c12', '#9b59b6'])
ax1.set_xticks(range(len(call_data)))
ax1.set_xticklabels([t.replace('Call ', '') for t in call_data['Type d\'Option']], rotation=45, ha='right')
ax1.set_ylabel('Prix ($)', fontsize=12, fontweight='bold')
ax1.set_title('Comparaison des Prix - Options CALL', fontsize=14, fontweight='bold', pad=15)
ax1.grid(True, alpha=0.3, axis='y')

# Ajout des valeurs sur les barres
for i, v in enumerate(call_data['Prix ($)']):
    ax1.text(i, v + 0.2, f'${v:.2f}', ha='center', va='bottom', fontweight='bold')

# Puts
put_data = comparison[comparison['Type d\'Option'].str.contains('Put')]
ax2.bar(range(len(put_data)), put_data['Prix ($)'], color=['#3498db', '#e74c3c', '#f39c12', '#9b59b6'])
ax2.set_xticks(range(len(put_data)))
ax2.set_xticklabels([t.replace('Put ', '') for t in put_data['Type d\'Option']], rotation=45, ha='right')
ax2.set_ylabel('Prix ($)', fontsize=12, fontweight='bold')
ax2.set_title('Comparaison des Prix - Options PUT', fontsize=14, fontweight='bold', pad=15)
ax2.grid(True, alpha=0.3, axis='y')

# Ajout des valeurs sur les barres
for i, v in enumerate(put_data['Prix ($)']):
    ax2.text(i, v + 0.2, f'${v:.2f}', ha='center', va='bottom', fontweight='bold')

plt.tight_layout()
plt.savefig('nintendo_options_comparison.png', dpi=300, bbox_inches='tight')
print("📈 Graphique sauvegardé: nintendo_options_comparison.png")
print()

# Graphique 2: Sensibilité au prix du sous-jacent
spot_range = np.linspace(current_price * 0.7, current_price * 1.3, 50)
european_calls = [black_scholes_european(S, strike_price, time_to_maturity, risk_free_rate, volatility, 'call') for S in spot_range]
european_puts = [black_scholes_european(S, strike_price, time_to_maturity, risk_free_rate, volatility, 'put') for S in spot_range]

fig, ax = plt.subplots(figsize=(14, 8))
ax.plot(spot_range, european_calls, label='Call Européen', linewidth=2.5, color='#27ae60')
ax.plot(spot_range, european_puts, label='Put Européen', linewidth=2.5, color='#e74c3c')
ax.axvline(x=current_price, color='gray', linestyle='--', linewidth=1.5, alpha=0.7, label=f'Prix actuel (${current_price:.2f})')
ax.axvline(x=strike_price, color='black', linestyle='-.', linewidth=1.5, alpha=0.7, label=f'Strike (${strike_price:.2f})')
ax.set_xlabel('Prix du Sous-jacent ($)', fontsize=12, fontweight='bold')
ax.set_ylabel('Prix de l\'Option ($)', fontsize=12, fontweight='bold')
ax.set_title('Sensibilité des Prix d\'Options au Prix du Sous-jacent', fontsize=14, fontweight='bold', pad=20)
ax.legend(loc='best', framealpha=0.9, fontsize=11)
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('nintendo_options_sensitivity.png', dpi=300, bbox_inches='tight')
print("📈 Graphique sauvegardé: nintendo_options_sensitivity.png")
print()

# Graphique 3: Surface de volatilité implicite (simplifié)
volatility_range = np.linspace(volatility * 0.5, volatility * 1.5, 30)
maturity_range = np.linspace(0.25, 2.0, 30)

call_prices = np.zeros((len(volatility_range), len(maturity_range)))

for i, vol in enumerate(volatility_range):
    for j, T in enumerate(maturity_range):
        call_prices[i, j] = black_scholes_european(current_price, strike_price, T, risk_free_rate, vol, 'call')

fig, ax = plt.subplots(figsize=(14, 8))
contour = ax.contourf(maturity_range, volatility_range * 100, call_prices, levels=20, cmap='viridis')
cbar = plt.colorbar(contour, ax=ax)
cbar.set_label('Prix du Call ($)', fontsize=11, fontweight='bold')
ax.set_xlabel('Maturité (années)', fontsize=12, fontweight='bold')
ax.set_ylabel('Volatilité (%)', fontsize=12, fontweight='bold')
ax.set_title('Surface des Prix de Call en fonction de la Volatilité et de la Maturité',
             fontsize=14, fontweight='bold', pad=20)
ax.scatter([time_to_maturity], [volatility * 100], color='red', s=200, marker='*',
           edgecolors='white', linewidths=2, label='Paramètres actuels', zorder=5)
ax.legend(loc='upper left', framealpha=0.9)

plt.tight_layout()
plt.savefig('nintendo_options_surface.png', dpi=300, bbox_inches='tight')
print("📈 Graphique sauvegardé: nintendo_options_surface.png")
print()

# ═══════════════════════════════════════════════════════════════════════════
# SECTION 9: RECOMMANDATIONS STRATÉGIQUES
# ═══════════════════════════════════════════════════════════════════════════

print("=" * 80)
print("  RECOMMANDATIONS STRATÉGIQUES POUR LE CLIENT")
print("=" * 80)
print()

print("🎯 ANALYSE COMPARATIVE DES TYPES D'OPTIONS:")
print()

print("1️⃣  OPTIONS EUROPÉENNES:")
print(f"   ✅ Avantages:")
print(f"      - Prix les plus bas (Call: ${european_call:.2f}, Put: ${european_put:.2f})")
print(f"      - Liquidité généralement meilleure sur les marchés")
print(f"      - Simplicité de pricing (Black-Scholes)")
print(f"   ❌ Inconvénients:")
print(f"      - Pas de flexibilité d'exercice anticipé")
print(f"      - Peut manquer des opportunités de profit")
print()

print("2️⃣  OPTIONS AMÉRICAINES:")
print(f"   ✅ Avantages:")
print(f"      - Flexibilité maximale (exercice à tout moment)")
print(f"      - Particulièrement intéressant pour les puts (protection)")
print(f"      - Prime d'exercice anticipé: Call +${early_exercise_premium_call:.4f}, Put +${early_exercise_premium_put:.4f}")
print(f"   ❌ Inconvénients:")
print(f"      - Prix plus élevés (Call: ${american_call:.2f}, Put: ${american_put:.2f})")
print(f"      - Moins liquides que les européennes")
print()

print("3️⃣  OPTIONS BERMUDÉENNES:")
print(f"   ✅ Avantages:")
print(f"      - Bon compromis flexibilité/coût")
print(f"      - Prix intermédiaires (Call: ${bermudan_call:.2f}, Put: ${bermudan_put:.2f})")
print(f"      - Convient aux stratégies trimestrielles")
print(f"   ❌ Inconvénients:")
print(f"      - Moins répandues, liquidité moindre")
print(f"      - Complexité de pricing")
print()

print("4️⃣  OPTIONS ASIATIQUES:")
print(f"   ✅ Avantages:")
print(f"      - Prix réduits (Call: ${asian_call:.2f}, Put: ${asian_put:.2f})")
print(f"      - Moins sensibles aux manipulations de prix")
print(f"      - Protection contre la volatilité de fin de période")
print(f"   ❌ Inconvénients:")
print(f"      - Payoff plus faible (basé sur moyenne)")
print(f"      - Moins liquides, marchés OTC principalement")
print()

print("=" * 80)
print("  RECOMMANDATIONS FINALES")
print("=" * 80)
print()

print("💼 STRATÉGIES RECOMMANDÉES PAR PROFIL:")
print()

print("📈 POUR UN INVESTISSEUR OPTIMISTE (bullish sur Nintendo):")
print("   → Option recommandée: CALL EUROPÉEN")
print(f"     • Prix: ${european_call:.2f}")
print(f"     • Levier maximal pour investissement minimal")
print(f"     • Perte limitée à la prime payée")
print()

print("📉 POUR UN INVESTISSEUR PRUDENT (protection du portefeuille):")
print("   → Option recommandée: PUT AMÉRICAIN")
print(f"     • Prix: ${american_put:.2f}")
print(f"     • Protection maximale avec flexibilité d'exercice")
print(f"     • Possibilité d'exercice anticipé si forte baisse")
print()

print("⚖️  POUR UN INVESTISSEUR ÉQUILIBRÉ:")
print("   → Option recommandée: CALL/PUT BERMUDÉEN")
print(f"     • Prix Call: ${bermudan_call:.2f}, Put: ${bermudan_put:.2f}")
print(f"     • Bon rapport flexibilité/coût")
print(f"     • Dates d'exercice trimestrielles")
print()

print("🎲 POUR UN INVESTISSEUR CHERCHANT RÉDUCTION DES COÛTS:")
print("   → Option recommandée: OPTIONS ASIATIQUES")
print(f"     • Prix Call: ${asian_call:.2f}, Put: ${asian_put:.2f}")
print(f"     • Primes réduites de ~{((european_call - asian_call)/european_call)*100:.1f}% (call) et ~{((european_put - asian_put)/european_put)*100:.1f}% (put)")
print(f"     • Convient aux horizons longs termes")
print()

print("=" * 80)
print("  POINTS CLÉS À RETENIR")
print("=" * 80)
print()

print("📌 Points importants:")
print(f"   1. Le marché valorise actuellement Nintendo avec une volatilité de {volatility*100:.2f}%")
print(f"   2. Les options américaines valent ~{(american_call/european_call - 1)*100:.1f}% de plus que les européennes (calls)")
print(f"   3. Les options asiatiques offrent une réduction de coût significative")
print(f"   4. Le choix optimal dépend de:")
print(f"      - L'objectif d'investissement (spéculation vs couverture)")
print(f"      - L'horizon temporel")
print(f"      - La tolérance au risque")
print(f"      - Le besoin de flexibilité d'exercice")
print()

print("⚠️  MISE EN GARDE:")
print("   - Ces calculs sont basés sur des modèles théoriques")
print("   - Les prix réels de marché peuvent différer")
print("   - Considérer les coûts de transaction dans la décision finale")
print("   - Consulter un conseiller financier pour toute décision d'investissement")
print()

print("=" * 80)
print("  FIN DE L'ANALYSE - OPTION PRICING")
print("=" * 80)